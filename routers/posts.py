...
from schemas import PostResponse, PostUpdate, PostCreate, PagniatedPostResponse

from auth import CurrentUser

router = APIRouter()

@router.get("", response_model=PagniatedPostResponse)
async def get_posts(db: Annotated[AsyncSession, Depends(get_db)],
                    skip: Annotated[int, Query(ge=0)] = 0,
                    limit: Annotated[int, Query(ge=1, le=100)] = 10):
    
    count_resutls = await db.execute(select(func.count()).select_from(models.Post))
    total = count_resutls.scalar_one() or 0
